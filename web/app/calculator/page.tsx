...
/* ─── Per-model feature weights: [creatinine, wbc, platelet, bilirubin, age] ─── */
const MODEL_FEATURE_WEIGHTS: Record<ModelId, [number, number, number, number, number]> = {
  gb:  [1.00, 1.00, 1.00, 1.00, 1.00], // balanced — best overall
  rf:  [0.90, 1.18, 1.08, 0.92, 0.92], // WBC / platelet emphasis
  ann: [0.95, 0.95, 0.93, 1.08, 1.09], // hepatic / age emphasis
  lr:  [1.12, 0.85, 0.80, 0.90, 1.13], // creatinine / age (interpretable linear)
};
...
/* ─── Risk estimation ─── */
function computeRisk(values: Record<string, number>, modelId: ModelId): RiskResult {
  const { creatinine, wbc, platelet, bilirubin, age } = values;
  const [wCr, wWbc, wPlt, wBil, wAge] = MODEL_FEATURE_WEIGHTS[modelId];

  let crScore = creatinine > 5 ? 35 : creatinine > 2.5 ? 22 : creatinine > 1.5 ? 12 : 3;
  let wbcScore = (wbc > 20 || wbc < 2) ? 25 : (wbc > 15 || wbc < 3.5) ? 15 : wbc > 11 ? 7 : 2;
  let pltScore = platelet < 50 ? 20 : platelet < 100 ? 12 : platelet < 150 ? 5 : 1;
  let bilScore = bilirubin > 5 ? 15 : bilirubin > 2 ? 8 : bilirubin > 1.2 ? 3 : 1;
  let ageScore = age > 80 ? 10 : age > 65 ? 6 : age > 50 ? 3 : 1;
...
export default function CalculatorPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setResult(computeRisk(values, selectedModel));
    setLoading(false);
  };
                  ...
                  {MODEL_OPTIONS.map((model) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black text-slate-900"
                  ...
                  >
                    <div className="relative">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="text-xs font-black" style={{ color: activeModel.color }}>
                          {activeModel.short}
                        </span>
                      </div>
                    </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              <span
                ...
                style={{ background: `${result.modelColor}20`, color: result.modelColor }}
              >
